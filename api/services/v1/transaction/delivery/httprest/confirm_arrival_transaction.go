...
type confimrArrivalTransactionResponseBody struct {
	Status string `json:"status"`
}

func (handler *transactionHTTPRestHandler) ConfirmArrivalTransaction(ctx *gin.Context) {
	...
	err, statusCode = handler.transactionUsecase.ConfirmArrivalTransaction(
		authUserID,
		transactionUUID,
	)
	...
	ctx.JSON(int(statusCode), confimrArrivalTransactionResponseBody{Status: "Success"})
