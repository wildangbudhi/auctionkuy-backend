...
type confimrPaymentTransactionRequestBody struct {
	PaymentMethodID *domain.UUID `json:"payment_method_id" binding:"required"`
	PaymentAccount  *string      `json:"payment_account" binding:"required"`
}

type confimrPaymentTransactionResponseBody struct {
	Status string `json:"status"`
}

func (handler *transactionHTTPRestHandler) ConfirmPaymentTransaction(ctx *gin.Context) {
	...
	requestBodyData := &confimrPaymentTransactionRequestBody{}
	...
	err, statusCode = handler.transactionUsecase.ConfimrPaymentTransaction(
		authUserID,
		transactionUUID,
		requestBodyData.PaymentMethodID,
		requestBodyData.PaymentAccount,
	)
	...
	ctx.JSON(int(statusCode), confimrPaymentTransactionResponseBody{Status: "Success"})
